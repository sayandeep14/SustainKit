...
Suggest ONE recipe for "${mealSlot}" using ONLY items from the inventory below (you may assume basic staples like water, oil, and salt are always available even if not listed).
...
Respond with JSON matching exactly this shape, and nothing else:
{
  ...
  "ingredients_used": [{ "inventory_item_id": string, "name": string, "quantity_used": string }],
  "instructions": [string, ...],
